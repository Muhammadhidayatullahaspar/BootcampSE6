...
public class GameController
{
	...
	public GameController(IPlayer player1, IPlayer player2)
	{
		...
		{
			{ player1, new PlayerData { playerPiece = InitialPiece.InitializePieces(12, PieceColour.White), moveKing = new List<Piece>(), status = PlayerStatus.OnGoing } },
			{ player2, new PlayerData { playerPiece = InitialPiece.InitializePieces(12, PieceColour.Black), moveKing = new List<Piece>(), status = PlayerStatus.OnGoing } }
		};
	...
	/// <summary>
	/// Mengambil status pemain berdasarkan pemain yang diberikan.
	/// </summary>
	/// <param name="player1">Objek pemain (IPlayer) pertama.</param>
	/// <param name="player2">Objek pemain (IPlayer) kedua.</param>
	/// <returns>Status pemain (PlayerStatus). Jika pemain tidak ditemukan, mengembalikan nilai default (0).</returns>
