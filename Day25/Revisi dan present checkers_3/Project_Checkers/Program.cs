	...
	static void Main()
	{
		...
		while (true)
		{
			...
			if (gameController.MovePiece(pieceToMove.Position, newPosition))
			{
				...
				gameController.CheckWinCondition(); // belum terpakai

				// Dapatkan status pemain setelah gerakan
				PlayerStatus statusPlayer1AfterMove = gameController.GetStatus(player1);
				PlayerStatus statusPlayer2AfterMove = gameController.GetStatus(player2);
