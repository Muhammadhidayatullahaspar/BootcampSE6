...
class Program
{
	static void Main()
	{
		...
		while (true)
		{
			...
			if (gameController.MovePiece(pieceToMove.Position, newPosition))
			{
				...
				// var statusPlayer2AfterMove = gameController.GetStatus(player2);
				var statusPlayer1AfterMove = gameController.GetStatus(player1, player2); // ini pakai satu parameter saja
	...
	private static Position GetPositionFromBoard(int boardPosition)
	{
		...
		int y = boardPosition / 8; // ini 8 pakai board.gamesize
