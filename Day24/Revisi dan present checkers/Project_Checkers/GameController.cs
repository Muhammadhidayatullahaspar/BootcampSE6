...
public class GameController
{
	...
	public GameController(IPlayer player1, IPlayer player2)
	{
		...
		foreach (var piece in player2Pieces)
		{
			...
			{
				player.AddPiece(piece); // player data, 
			}
		}
	...
	{ // ini jangan pakai tupple, terus buatkan kelas masing masing untuk dapatkan color, piece id, piecetype
	...
	public PlayerStatus GetStatus(IPlayer player1, IPlayer player2) // ini sudah ada revisi seperti dibawah
	{
		...
		if (IsValidMove(piece, from, to))
		{
			...
			if (Math.Abs(to.x - from.x) == 2 && Math.Abs(to.y - from.y) == 2)
			{
				...
				if (capturedPiece != null)
				{
					...
					if (_players[capturedPiece.Player].Player is Player player) // perbaiki iplayer
					{
	...
	{
		CheckWinCondition(); // ini sepertinya tida berguna
	}
	#endregion
