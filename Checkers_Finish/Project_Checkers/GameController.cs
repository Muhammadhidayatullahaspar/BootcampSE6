		...
		if (IsValidMove(piece, from, to))
		{
			...
			if (Math.Abs(to.X - from.X) == 2 && Math.Abs(to.Y - from.Y) == 2)
			{
				...
				if (capturedPiece != null)
				{
					...
					{
						if (player is Player actualPlayer && actualPlayer.Pieces.ContainsKey(capturedPiece.pieceId))
						{
							actualPlayer.RemovePiece(capturedPiece.pieceId);
							break; // Keluar dari loop setelah menemukan dan menghapus potongan
						}
					}
				}
